...
clientPromise.then(() => {
  ...
  console.error("   For a quick test, you can add `0.0.0.0/0` (Allow Access From Anywhere), but this is not recommended for long-term production.");

  console.error("\n2. VERIFY MONGODB_URI IN YOUR `.env` FILE:");
  console.error("   - Double-check that the username, password, and cluster address are correct.");
  console.error("   - Ensure there are no special characters in the password that need to be URL-encoded.");
